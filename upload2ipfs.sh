...
reduce_video_if_needed() {
    ...
    local current_dims
    current_dims=$(ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 "$video_file" 2>/dev/null | head -n 1 | tr -d '\n\r')
    ...
    local size_ratio reduction_factor
    size_ratio=$(echo "$current_size $target_size" | awk '{printf "%.3f", $1 / $2}')
    reduction_factor=$(echo "$size_ratio" | awk '{printf "%.3f", sqrt($1)}')
    ...
    new_width=$(echo "$current_width $reduction_factor" | awk '{w = int($1 / $2); if (w % 2 != 0) w = w - 1; print w}')
    new_height=$(echo "$current_height $reduction_factor" | awk '{h = int($1 / $2); if (h % 2 != 0) h = h - 1; print h}')

    [[ $new_width -lt 320 ]]  && new_width=320
    [[ $new_height -lt 240 ]] && new_height=240
    [[ $((new_width  % 2)) -ne 0 ]] && new_width=$((new_width  - 1))
    [[ $((new_height % 2)) -ne 0 ]] && new_height=$((new_height - 1))
    ...
    if [ "$ffmpeg_ok" -eq 0 ] && [[ -f "$temp_output" ]] && [[ -s "$temp_output" ]]; then
        local new_size
        new_size=$(stat -c%s "$temp_output" 2>/dev/null || echo "0")
        if [[ $new_size -gt 0 ]] && [[ $new_size -lt "$current_size" ]]; then
            if mv "$temp_output" "$video_file" 2>/dev/null; then
                echo "DEBUG: ✅ Video resized: ${current_width}x${current_height} -> ${new_width}x${new_height}, $current_size -> $new_size bytes" >&2
                FILE_SIZE=$new_size
                return 0
            fi
        fi
        echo "WARNING: Resized file not smaller or move failed" >&2
        rm -f "$temp_output"
        return 1
    else
...
INFO_JSON_CONTENT="{
  \"protocol\": {
    ...
    \"specification\": \"https://github.com/papiche/Astroport.ONE/blob/master/docs/UPlanet_FILE_CONTRACT.md\"
  },
  \"file\": {
    \"name\": \"$FILE_NAME\",
    \"size\": $FILE_SIZE,
